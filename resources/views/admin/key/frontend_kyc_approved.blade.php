...
<div class="container mt-4">
    <div class="card border-0 shadow">
        <div class="card-header bg-success text-white">
            <h4 class="mb-0"><i class="fa-solid fa-check-circle me-2"></i> Approved KYC List</h4>
        </div>
        <div class="card-body">
            @php
                $kycs = \App\Models\Kyc::with('user')->where('status', 'approved')->latest()->get();
            @endphp

            @if($kycs->isEmpty())
                <div class="alert alert-info text-center">No approved KYC found.</div>
            @else
